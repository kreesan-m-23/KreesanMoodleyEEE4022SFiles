% Audio Radar Project - Generate Range-Doppler Map
...
% Target parameters 
R_target = 5;                       % Target range 
Vel_target = -0.10;                  % Target radial velocity in m/s
R_target2 = 8;                       % Target range 
Vel_target2 = 0.08;                  % Target radial velocity in m/s
...
%% Exercise 6: Generate the range line for multiple pulses, assuming 2 moving targets

% Generate the Received Signal

Rx_Signal1 = zeros(1, size(t,2));

for Count_PulseNum = 1: NumPulses
    tdn = 2*(R_target - Vel_target*PRI*(Count_PulseNum - 1))/c + PRI*(Count_PulseNum - 1); 
    Rx_Signal1 = Rx_Signal1 +  cos(2*pi*(fc*(t - T/2 - tdn) + 0.5*K*(t - T/2 - tdn).^2) ).* rect( (t - T/2 - tdn)/T );  % received signal   
end;

for Count_PulseNum = 1: NumPulses
    tdn = 2*(R_target2 - Vel_target2*PRI*(Count_PulseNum - 1))/c + PRI*(Count_PulseNum - 1); 
    Rx_Signal1 = Rx_Signal1 +  cos(2*pi*(fc*(t - T/2 - tdn) + 0.5*K*(t - T/2 - tdn).^2) ).* rect( (t - T/2 - tdn)/T );  % received signal   
end;
...
output = abs((fftshift(fft(Window_Matrix.*MF,[],1),1)));

% Plot Received Signal Results
figure(1)
subplot(2,1,1);
plot(t,Tx_Signal);
xlabel('Time (s)');
...
figure(4)
subplot(2,1,1);
plot(frequencyaxis2,abs(fftshift(fft(Tx_Signal))));
xlabel('Frequency');
ylabel('Magnitude (dB)');
...
% Plot Output of Matched Filtering Operation
figure(5)
imagesc(RangeLineAxis_m, Velocity_cmpersec, abs(MF));
colorbar;
clims= [-500 500];
grid On;
ax = gca;
...
figure(6)
imagesc(RangeLineAxis_m, Velocity_cmpersec, output);
colorbar;
clims= [-500 500];
grid On;
ax = gca;
